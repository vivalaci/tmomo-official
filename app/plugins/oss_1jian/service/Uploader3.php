...
require_once './application/plugins/oss_1jian/oss/act/Common.php';
use OSS\OssClient;
...
private $hash;
    ...
    private $stateMap = array( //上传状态映射表，国际化用户需考虑此处数据的国际化
        ...
        "OSS_CONNECT_ERROR" => "oss连接错误",
        "ERROR_GET_REMOTE_FILE"=>"上传远程图片错误"
    );
    ...
    public function __construct($fileField, $config, $type = "file")
    {
        ...
        $this->ossClient =\Common::getOssClient($OsscConfig["OSS_ACCESS_ID"],$OsscConfig["OSS_ACCESS_KEY"],$OsscConfig["OSS_ENDPOINT"]);
...
if (is_null($this->ossClient)) 
{
    ...
    return;
        ...
        if(!empty($_FILES[$this->fileField]))
        {
            ...
            $this->hash = hash('sha256', $file['tmp_name'],false);;
    ...
    private function uploadFile()
    {
        ...
        try{
            $content = '';
            $fp = fopen($file["tmp_name"], 'r'); //这里就是input中类型是file名字是file
            if ($fp) {
            while (!feof($fp)) {
           ...
           // $r=$ossClient->uploadFile($bucketName, $object, "exa$objemple.jpg");
            ...
            $this->stateInfo = $this->getStateInfo("ERROR_GET_REMOTE_FILE");
              return;
            }
if($r!="")
        $r=\Common::object_array($r);
...
if($r!="")
    ...
    private function saveRemote()
    {
        ...
        $this->fileSize = strlen($img);
        $this->fileType = $this->getFileExt();
        $this->fullNamefullName = $this->getFullName();
        $this->filePath = $this->getFilePath();
        $this->fileName = $this->getFileName();
        $dirname = dirname($this->filePath);
...
$r="";
try{
    $object= $this->fullNamefullName;
   // $r=$ossClient->uploadFile($bucketName, $object, "example.jpg");
    $r=$this->ossClient->putObject($this->bucketName, $object,$img);
    }
    catch(OssException $e) {
...
$this->stateInfo = $this->stateMap[0];
   ...
   $format=str_replace("/public/static","/oss",$format);
    ...
    public function getFileInfo()
    {
        ...
        if(empty($OSS_DOMAIN) )
        {
            $OSS_DOMAIN="http://".$data["OSS_TEST_BUCKET"].".".$data["OSS_ENDPOINT"];

        }
        return array(
            "state"     => $this->stateInfo,
            "url"       => $OSS_DOMAIN.$this->fullName,
            "path"      => $this->filePath,
            "title"     => $this->fileName,
