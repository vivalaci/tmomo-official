...
use app\plugins\oss_1jian\service\Uploader;
...
class UeditorService
{
    ...
    {
     $data=input();
   $attachment=  Db::name('attachment')->where('id',$data["id"])->find();
if($attachment==null || count($attachment)<=0)
{
    self::$current_result = [
        'state'=> "ERROR"
    ];
...
$url=$attachment["url"];
  ...
  if(strstr($url,"http"))
  {
   ...
   $bucketName = $OsscConfig["OSS_TEST_BUCKET"];

   $objects = array();
   $objects[] =$path;

   $ossClient =\Common::getOssClient($OsscConfig["OSS_ACCESS_ID"],$OsscConfig["OSS_ACCESS_KEY"],$OsscConfig["OSS_ENDPOINT"]);
   ...
   $r=$ossClient->deleteObjects($bucketName, $objects);
...
Db::name('attachment')->where('id',$data["id"])->delete();
  }
  else {
