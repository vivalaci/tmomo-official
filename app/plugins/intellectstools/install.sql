...
) ENGINE=InnoDB DEFAULT CHARSET={CHARSET} ROW_FORMAT=DYNAMIC COMMENT='智能工具箱 - 订单备注';
...
) ENGINE=InnoDB DEFAULT CHARSET={CHARSET} ROW_FORMAT=DYNAMIC COMMENT='智能工具箱 - 商品备注';
...
CREATE TABLE IF NOT EXISTS `{PREFIX}plugins_intellectstools_comments` (
  ...
  KEY `content` (`content`)
) ENGINE=InnoDB DEFAULT CHARSET={CHARSET} ROW_FORMAT=DYNAMIC COMMENT='智能工具箱 - 批量评价';
...
) ENGINE=InnoDB DEFAULT CHARSET={CHARSET} ROW_FORMAT=DYNAMIC COMMENT='智能工具箱 - 批量评价商品增加记录';
...
CREATE TABLE IF NOT EXISTS `{PREFIX}plugins_intellectstools_comments_goods_config` (
  ...
  PRIMARY KEY (`id`),
  KEY `goods_id` (`goods_id`)
) ENGINE=InnoDB DEFAULT CHARSET={CHARSET} ROW_FORMAT=DYNAMIC COMMENT='智能工具箱 - 批量评价商品独立配置';
...
CREATE TABLE IF NOT EXISTS `{PREFIX}plugins_intellectstools_toutiaosettlement` (
  ...
  `status` tinyint(20) unsigned NOT NULL DEFAULT '0' COMMENT '分账状态（0待分账, 1已分账, 2已失败）',
  `is_push` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否推送（0否, 1是）',
