...
class ExpressQueryService
{
    ...
    {
    //   echo "=== 开始请求物流数据 ===<br/>";
    // echo "快递类型: ".$config['express_type']."<br/>";
        ...
        switch($config['express_type'])
        {
            ...
            case 'shenzhou':
                $res = self::ShenzhouQuery($config, $order, $express);
                break;
                
            default :
                return DataReturn('快递类型未定义['.$config['express_type'].']', -1);
        }
//  echo "查询方法返回结果: ".print_r($res, true)."<br/>";
        // 数据添加
        $insert_data = [
 ...
 * 神州集运数据查询
 */
public static function ShenzhouQuery($config, $order, $express)
{

    
    // 参数验证
    if (empty($express['express_number'])) {
        echo "错误：快递单号为空<br/>";
        return [
            'result_data' => DataReturn('快递单号不能为空', -1),
            'request_params' => []
        ];
    }

    // 准备请求
    $url = 'https://api.earlybird.network/check-order-status-tmomo';
    $request_params = ['order_num' => $express['express_number']];
    ...
    if ($res['code'] == 0 && !empty($res['data'])) {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            $result['msg'] = '响应JSON解析失败';
            echo "JSON解析错误: ".json_last_error_msg()."<br/>";
        } elseif (!empty($response) && is_array($response)) {
          
            
            if (isset($response['code']) && $response['code'] == 1 && !empty($response['traces'])) {
                ...
                foreach ($response['traces'] as $trace) {
                    $data[] = [
                        ...
                        'desc' => ($trace['description'] ?? '') . 
                                 (empty($trace['location']) ? '' : ' ['.$trace['location'].']')
                    ];
                }
                ...
                $result['msg'] = $response['msg'] ?? 'API返回无物流数据';
               
            }
        }
    } else {
        $result['msg'] = $res['msg'] ?? 'API请求失败';
        echo "请求失败: ".$result['msg']."<br/>";
    }
