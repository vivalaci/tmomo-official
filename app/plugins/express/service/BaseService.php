...
class BaseService
{
    ...
    {
        // 仅已发货、已完成，销售模式展示
        if(isset($params['status']) && in_array($params['status'], [3,4]) && isset($params['order_model']) && in_array($params['order_model'], [0,2]))
        {
            // 订单已完成、是否可以满足限定时
